...
class PatientService {
   ...
   * Uses the view to get location names included
  ...
  async searchSupabasePatients(
    ...
    success: boolean;
    data: PatientProfileWithLocations[];
    count: number;
    message?: string;
  }> {
    try {
      ...
      // Search using the view that includes location names
      ...
      const enrichedData = await Promise.all(
        (profiles || []).map(async (profile) => {
          ...
          } as PatientProfileWithLocations;
        })
      );
